...
function bytesToNumber(bytes: any) {
...
function pointAddScalar(p: any, tweak: any, isCompressed: any) {
...
function pointMultiply(p: any, tweak: any, isCompressed: any) {
  const P = necc.Point.fromHex(p);
...
function privateNegate(privateKey: any) {
  const p = normalizePrivateKey(privateKey);
